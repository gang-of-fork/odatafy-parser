...
export function parseMathExpression(mathExpr: string) {
    ...
    let [strResult, strResolver] = escapeStrings(mathExpr);
    let [funcResult, funcResolver] = escapeFunctions(strResult as string);
    let preparedMathExpr = replaceMathSymbols(funcResult);
    let mathExprAst = parse(preparedMathExpr);

    //@ts-ignore unspecified types from lib
    let resolvedAst = mathExprAst.transform((node, _path, _parent) => {
        if (node.type == 'SymbolNode') {
            let symbolNode = (<math.SymbolNode>node)
...
export function cleanAST(ast: any): FilterNode {
    ...
    switch (ast.mathjs) {
        case NodeTypes.OperatorNode:
            ...
            cleanedNode.op = <OperatorNodeOperators>cleanedNode.op
        ...
        case NodeTypes.ConstantNode:
            cleanedNode = {
                ...
                type: Number.isInteger(ast.value) ? ConstantNodeTypes.Integer : ConstantNodeTypes.Decimal,
                value: ast.value
            }
            break;
